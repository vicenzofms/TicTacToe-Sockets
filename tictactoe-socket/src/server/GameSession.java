...
public class GameSession {
    ...
    private synchronized void handleMove(PlayerHandler player, String value) {
        ...
        if (player.symbol != currentTurn) {
            player.send(Protocol.command(Protocol.ERROR, "Ainda não é a sua vez."));
            sendState();
            return;
        }
        ...
        try {
            ...
            player.send(Protocol.command(Protocol.ERROR, "Jogada inválida. Use um número de 0 a 8."));
            return;
        }

        if (!board.makeMove(position, player.symbol)) {
            player.send(Protocol.command(Protocol.ERROR, "Posição inválida ou ocupada."));
            sendState();
            return;
        }
